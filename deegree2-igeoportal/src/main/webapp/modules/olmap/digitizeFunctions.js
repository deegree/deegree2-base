...
function initDrawinPane() {
    ...
    if (  tmp.length == 0 ) {
        ...
        drawingPane = new OpenLayers.Layer.Vector( "_DIGITIZE_FEATURES_", { styleMap: initDigitizerStyles() });
...
function initModifyFeature(callback) {
    ...
    var control = new OpenLayers.Control.ModifyFeature( drawingPane,{ 
						        beforefeaturemodified: function(feature) { callback( feature );  } 
						    }  );
    map.addControl( control );
    control.activate();
}
...
function initSelectFeature(callback) {
    ...
    var control = new OpenLayers.Control.SelectFeature( drawingPane,{ 
						        onSelect: function(feature) { callback( feature );  } 
						    }  );
    map.addControl( control );
    control.activate();
}
